...
class VIEScore:
    def __init__(self, backbone="gpt4o", task="t2i", key_path=None, azure_endpoint='') -> None:
        ...
        if self.backbone_name == "gpt4o" or self.backbone_name == "gpt-4.1":
            from viescore.mllm_tools.openai import GPT4o
            # self.model = GPT4o(key_path, model_name="gpt-4.1-2025-04-14", azure_endpoint=azure_endpoint)
            self.model = GPT4o(key_path, model_name="gpt-4.1", azure_endpoint=azure_endpoint)
        ...
        if self.task == "t2i":
            ...
            self.SC_prompt = "\n".join([self.context, vie_prompts._prompts_0shot_two_image_edit_rule_ours, vie_prompts._prompts_0shot_tie_rule_SC_ours])
            self.PQ_prompt = "\n".join([self.context, vie_prompts._prompts_0shot_rule_PQ_ours])
        elif self.task == "t2v":
    ...
    def evaluate(self, image_prompts, text_prompt, extract_overall_score_only=False, extract_all_score=True, echo_output=False):
        ...
        if extract_all_score:
            ...
            w = 0.7  
            O_score = (SC_score ** w) * (PQ_score ** (1 - w))
