...
def _get_input_embeds(
    ...
    if pixel_values is not None:
        ...
        # [NEW] 缺图像时：用极小 dummy 输入“点亮”图像分支（接入计算图但不改变数值）
        config = model.config.vision_config
        patch_dim = config.in_channels * config.temporal_patch_size * (config.patch_size ** 2)
        ...
        inputs_embeds = inputs_embeds + 0.0 * _dummy_img_embeds.mean()  # 数值不变，但图/梯度接上
    ...
    if pixel_values_videos is not None:
        ...
        # [NEW] 缺视频时：用极小 dummy 输入“点亮”视频分支（接入计算图但不改变数值）
        config = model.config.vision_config
        patch_dim = config.in_channels * config.temporal_patch_size * (config.patch_size ** 2)
        ...
        inputs_embeds = inputs_embeds + 0.0 * _dummy_vid_embeds.mean()  # 数值不变，但图/梯度接上

    # [REMOVED] 原先 “两个都 None 时再兜底” 的块已不需要
    # if pixel_values is None and pixel_values_videos is None:
    #     config = model.config.vision_config
    #     patch_dim = config.in_channels * config.temporal_patch_size * config.patch_size**2
    #     pixel_values = torch.zeros((16, patch_dim), dtype=inputs_embeds.dtype, device=inputs_embeds.device)
