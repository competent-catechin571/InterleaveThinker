...
def api(prompt, model, kwargs={}):
    import dashscope
    api_key = os.environ.get('DASHSCOPE_API_KEY', 'sk-d4d3a3f33cd74171ba818cedef88099a')
    if not api_key:
...
def edit_api(prompt, img_list, model="qwen-vl-max-latest", kwargs={}):
    import dashscope
    api_key = os.environ.get('DASH_API_KEY', 'sk-d4d3a3f33cd74171ba818cedef88099a')
    if not api_key:
